...
grafico2<-ggplot(ds, mapping=aes(x=MinTemp, y=MaxTemp, color=RainTomorrow))
grafico3<-ggplot(ds, mapping=aes(x=MinTemp, y=MaxTemp, color=RainTomorrow)) + geom_point() + ggtitle("Gráfico dispersión MaxTemp y MinTemp")
...
grafico1_2<- ggplot(ds, aes(x=Temp9am, y= Pressure9am, color=RainToday)) + geom_point() + labs(title="Gráfico dispersión 2", subtitle="Temp9am y Pressure9am", x="Temperatura a las 9am", y="Presión a las 9am", color="Llovio hoy?")
...
iris.grafico2<-ggplot(iris.tidy, aes(x=Species, y=Value, color=Part)) + geom_point()+ facet_grid(vars(Measure) )

iris.grafico3<-ggplot(iris.tidy, aes(x=Species, y=Value, color=Part)) + geom_jitter() + facet_grid(vars(Measure) )
...
grafico1_4<-ggplot(DatosEjer4,aes(x=Fecha, y=Ventas, color=Fabricante)) + geom_point()

grafico2_4<-ggplot(DatosEjer4,aes(x=Fecha, y=Ventas, color=Fabricante)) +
  geom_line(color="red", )

grafico3_4<-ggplot(DatosEjer4, aes(x=Fecha, y=Ventas, color=Fabricante, linetype=Fabricante)) + geom_point() +
  geom_line() + labs(title="Ventas mensuales de diferentes compañías tecnológicas")
...
grafico51_5<-ggplot(Experimento, aes(x=Dosis, y=Altura, linetype=Complemento)) + geom_point()+geom_line()

grafico2_5<-ggplot(Experimento, aes(x=Dosis, y=Altura, linetype=Complemento)) + geom_point()+geom_line()+geom_errorbar(aes(ymin=min(Experimento[['Error_Altura']]), ymax=max(Experimento[['Error_Altura']])))

grafico2_5

grafico3_5 <- ggplot(Experimento, aes(x=Dosis, y=Altura, color=Complemento)) +
  geom_point(size=3, shape=16) +
  geom_line(linewidth=1) +
  geom_errorbar(aes(ymin = Altura - Error_Altura, ymax = Altura + Error_Altura),
                width = 0.1) +
  scale_color_brewer(palette="Reds") +
  labs(title="Altura frente a dosis con barras de error (paleta roja)",
       x="Dosis (mg)", y="Altura (cm)",
       color="Complemento") +
  theme_minimal()

grafico3_5

ggsave("figs/Ejercicio5.pdf", plot = grafico3_5, width = 8, height = 5)
...
grafico1_6<-ggplot(ice.river.df, aes(x=time))+geom_line(aes(y=flow.vat,color='red'))+geom_line(aes(y=flow.jok,color='blue'))
...
grafico1_7 <- ggplot(ds, aes(x = factor(Year), y = MinTemp)) +
  geom_boxplot(fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de temperaturas mínimas por año",
       x = "Año",
       y = "Temperatura mínima (ºC)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

grafico1_7

grafico2_7 <- ggplot(ds, aes(x = factor(Year), y = MaxTemp, fill = factor(Year))) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1)  # Rotamos etiquetas del eje X
  )
...
chol <- read.table("http://assets.datacamp.com/blog_assets/chol.txt", header = TRUE)

# ggplot(data=chol, aes(x=HEIGHT)) +
...
ggplot(data = chol, aes(x = HEIGHT)) +
  geom_histogram(
    breaks = seq(155, 195, length.out = 21),  # 20 bins → 21 puntos de corte
    fill = "blue",
    color = "red",
    alpha = 0.5
  ) +
  geom_density(aes(y = ..count..), color = "green", linewidth = 1) +  # curva de densidad
  coord_cartesian(xlim = c(155, 195)) +  # rango del eje x
  labs(
